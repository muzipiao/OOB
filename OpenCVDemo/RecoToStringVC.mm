...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    ...
    if (brightnessValue < 1.5) {
        // 局部自适应快速积分二值化方法
        dstBinMat = adaptiveBinThereshold(imgMat);
    }else{
        // 转换为灰度图像,加快处理速度
    ...
    // 缩放图片尺寸
    CGFloat imgW = 15;
    CGFloat imgH = (15*dstImage.size.height)/dstImage.size.width;
...
cv::Mat adaptiveBinThereshold(cv::Mat src)
{
    cv::Mat dst;
